...
`vercel.json`이 `api/index.py`(Flask 앱)를 단일 서버리스 함수로 라우팅하도록 설정되어 있습니다.

**회사 목록 데이터**: DART의 전체 회사 고유번호 목록(corpCode.xml, 약 30MB, 11만개+)을 매 요청마다 새로 받으면 서버리스 환경의 실행시간 제한을 넘기므로, 미리 정제해 `dart_lib/data/corp_code.json.gz`(약 1.6MB)로 저장소에 포함시켜 사용합니다. 신규 상장/신설 법인을 반영하려면 주기적으로 아래 명령으로 갱신 후 커밋하세요.

```
python -m dart_lib.corp_code --refresh
```

## 한계사항

- 연결재무제표(CFS) + 사업보고서(연간) 기준으로만 조회합니다. 별도재무제표나 분기/반기 보고서는 지원하지 않습니다.
- 비상장 계열사 등 "사업보고서"를 제출하지 않고 "감사보고서"만 제출하는 회사는 전 연도가 "N/A"로 표시됩니다. DART의 전체 재무제표 API(`fnlttSinglAcntAll.json`)가 사업보고서 기준 데이터만 제공하기 때문으로, 코드로 해결할 수 없는 한계입니다 (예: 롯데바이오로직스).
...
- 신설법인이거나 위 사유(감사보고서만 제출 등)로 특정 연도 데이터가 없으면 해당 항목은 "N/A"로 표시되고, 실행 종료 시(웹은 화면 하단 "주의사항") 경고 목록에 사유가 함께 안내됩니다.
- 회사 목록은 위 스냅샷 기준이라, 아주 최근(스냅샷 이후) 신규 상장/상호변경 회사는 검색되지 않을 수 있습니다.
